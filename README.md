...
| 5 | **三層合約測試 (Contract Testing)** | InMemory Fake 行為跟 Real 不會偏離、事件 schema 改動有人擋、REST API 改了會通知所有 consumer |
...
## 6. 核心概念四:合約測試 (Contract Testing)

### 6.1 為什麼需要「合約」測試

整合測試告訴你「Adapter 跟外部系統真的對得起來」,但有幾種錯它擋不掉:

| 問題 | 整合測試擋不掉 | 合約測試擋得掉 |
|---|---|---|
| InMemory Fake 跟 Real JPA 行為長期偏離 | 兩邊各跑各的,沒人比對 | 同一份 Contract 兩邊都跑 |
| Integration Event 改了欄位名,接收 Context 默默壞掉 | 各 Context 各自測,沒人擋 | 事件 schema 鎖住,改了立刻紅 |
| REST API 改 response 結構,mobile client 過幾天才發現 | 後端覺得「向下相容啊」,前端說「我這邊壞了」 | Consumer 寫的契約綁住 provider |

合約測試的核心:**把「兩個東西要相容」這件事用程式碼鎖住,任何一端改了不相容的地方,測試立刻紅**。

本專案實作三層合約測試,從最近到最遠:

```
┌─────────────────────────────────────────────────────────────┐
│ Layer 1: Port Contract                  ← 同進程,跨 Adapter  │
│   InMemory Fake ≡ JPA Real                                  │
├─────────────────────────────────────────────────────────────┤
│ Layer 2: Event Schema Contract          ← 同進程,跨 Context  │
│   OrderCreatedEvent 的 JSON 結構不會無聲變更                  │
├─────────────────────────────────────────────────────────────┤
│ Layer 3: Consumer-Driven Contract (Pact) ← 跨進程             │
│   mobile-app 對 orders-api 的期望                            │
└─────────────────────────────────────────────────────────────┘
```

### 6.2 Layer 1 — Port Contract Test(內部,跨 Adapter)

每個 Outbound Port 都有兩種 Adapter:
- **Real**(JPA + Testcontainer):prod 用、IT 用
- **InMemory Fake**:Application 層 unit test 用

問題:如果 InMemory 跟 JPA 行為有差(例如 InMemory `save()` 直接回原物件,JPA 會重新 hydrate 並回傳 detached copy),「unit test 綠 → IT 紅」會浪費你很多時間。

**解法:抽象契約類 + 子類各補上 `repository()`**
...
// InMemory 端 — 跟著 unit test 跑 (./gradlew test)
class InMemoryOrderRepositoryContractTest extends OrderRepositoryContract {
    @Override protected OrderWriteRepository repository() {
        return new InMemoryOrderRepository();
    }
}

// JPA 端 — 跟著 IT 跑 (./gradlew integrationTest,需 Docker)
@DataJpaTest @Testcontainers
class JpaOrderRepositoryContractIT extends OrderRepositoryContract {
    @Container static PostgreSQLContainer<?> PG = new PostgreSQLContainer<>("postgres:16-alpine");
    @Autowired JpaOrderRepository jpa;
    @Override protected OrderWriteRepository repository() { return jpa; }
}
```

任何一條 assertion 在 InMemory 跟 JPA 跑出不同結果,就是 Fake 失去 prod 的代表性,**它倆同一份契約之下不可能各跑各的**。

本專案三個 Bounded Context 各有一份契約:
- `product/src/test/java/com/tutorial/ecommerce/product/contract/OrderRepositoryContract.java`
- `payment/src/test/java/com/tutorial/ecommerce/payment/contract/PaymentRepositoryContract.java`
- `inventory/src/test/java/com/tutorial/ecommerce/inventory/contract/StockWriteRepositoryContract.java`

每個契約都有 `InMemory*ContractTest` 與 `Jpa*ContractIT` 兩個子類,共 6 個檔。

### 6.3 Layer 2 — Event Schema Contract(內部,跨 Context)

Integration Event 是 Bounded Context 之間的「契約」。一旦 `OrderCreatedEvent` 發出去,Payment Context 就照著 schema 解碼。如果發送端把 `totalAmount` 改成 `total_amount`,接收端會默默壞掉 — 但不會在編譯期報錯,因為兩個 Context 沒有直接 import,只透過事件解耦。

**解法:把事件序列化後的「公開欄位路徑」鎖死**

```java
// shared-kernel/src/test/java/.../contract/IntegrationEventSchemaContractTest.java
@Test
void orderCreatedEvent_schema() throws Exception {
    var event = new OrderCreatedEvent(orderId, buyerId, lines, totalAmount, occurredAt);
    var paths = EventJsonFlattener.fieldPaths(
...
第一次跑這個契約會發現意想不到的事 — Jackson 把 `Money.isNegative()` 序列化成 `negative` 欄位,把 `Quantity.isZero()` 變成 `zero` — 等於把實作細節公開到契約上。這正是契約測試最值錢的地方:**把「真正暴露的東西」列出來**,而不是你以為的「乾淨欄位清單」。

修法:加 `@JsonIgnore` 或改方法命名。但在動手之前,先讓契約測試把所有現況都暴露出來,你才知道哪些 consumer 已經依賴這些欄位、不能說刪就刪。

實際檔案:
- `shared-kernel/src/test/java/com/tutorial/ecommerce/sharedkernel/contract/IntegrationEventSchemaContractTest.java` — 5 個事件的 schema
- `shared-kernel/src/test/java/com/tutorial/ecommerce/sharedkernel/contract/EventJsonFlattener.java` — 把 JSON 樹攤平成欄位路徑清單的小工具

### 6.4 Layer 3 — Consumer-Driven Contract(跨進程,Pact)

當 OrderController 對外提供 REST API,前端(mobile app、web、第三方)各自寫程式呼叫。如果有一天你重構 response — 把 `orderId` 改成 `id` — 前端就壞了。

**Consumer-Driven Contract** 翻轉責任:**Consumer(前端)寫他期待 provider 怎麼回應,變成測試,讓 provider(後端)跑這份測試保證自己沒打壞**。

[Pact](https://pact.io/) 是這個模式的事實標準工具。流程:

```
 mobile-app 團隊                       orders-api 團隊 (這個 repo)
─────────────                          ───────────────────────
 1. 寫消費者測試 (Pact consumer test)
    描述「我打 POST /api/orders 期望
    收到 201 + {orderId: ...}」
       │
       ▼
 2. 產出 pact 檔 (JSON)
    推到 Pact Broker
                                       3. 拉 pact 檔下來
                                          │
                                          ▼
                                       4. Pact provider test
                                          啟動 orders-api,
                                          用真的 HTTP 重放每個 interaction
                                          ↳ 任何 response 不符,測試紅
```

本專案實作 step 3-4(provider 端),consumer 端假設由 mobile-app 團隊維護。

Pact 檔在 `app/src/test/resources/contracts/pacts/mobile-app-orders-api.json`:
...
```json
{
  ...
  "interactions": [{
    "description": "place an order succeeds and returns the order id",
    "providerStates": [{ "name": "buyer is authenticated and can place an order" }],
    "request":  {
      "method": "POST", "path": "/api/orders",
      "headers": { "Authorization": "Bearer pact-stub-token" },
      "body": { "lines": [...] }
    },
    "response": {
      "status": 201,
      ...
      "matchingRules": {
        "body": {
          "$.orderId": { "matchers": [{ "match": "regex", "regex": "[0-9a-fA-F-]+" }] }
        }
      }
    }
  }, ...]
...
Provider 端的驗證 — `app/src/test/java/com/tutorial/ecommerce/pact/OrderApiPactProviderIT.java`:
...
class OrderApiPactProviderIT {
    ...
    @MockitoBean PlaceOrderUseCase placeOrder;  // 避免每次都真的觸發事件鏈

    @TestTemplate
    @ExtendWith(PactVerificationInvocationContextProvider.class)
    void verifyContract(PactVerificationContext ctx) { ctx.verifyInteraction(); }

    @State("buyer is authenticated and can place an order")
    void givenAuthenticatedBuyer() {
        when(placeOrder.handle(any())).thenReturn(
            new OrderId(UUID.fromString("22222222-2222-2222-2222-222222222222")));
    }

    // 測試版 JwtDecoder:接受任何 token,讓 pact 檔可以用固定字串
    @Configuration
    static class PactSecurityOverride {
        @Bean @Primary
        JwtDecoder pactJwtDecoder() { /* 回傳固定的合法 Jwt */ }
    }
}
```

關鍵設計:
- **`@State` 方法呼應 pact 檔的 `providerStates`**:每個 interaction 跑之前,讓 provider 端先 setup 必要狀態(mock 回傳值、DB 種子資料等)
- **`@MockitoBean` 替換 PlaceOrderUseCase**:避免 contract 測試副作用,專注在 HTTP 層面的契約
- **`JwtDecoder` 用測試版本**(`@Primary` Bean 覆寫):接受任何 token,讓 pact 檔的 `Bearer pact-stub-token` 過得了驗證
- **matchingRules**:`orderId` 用 regex 比對,因為實際 UUID 每次都不一樣 — 比的是 **shape**,不是 exact value

跑法:`./gradlew :app:integrationTest --tests "*PactProvider*"`,需要 Docker。

### 6.5 為什麼一定要是「Consumer-Driven」

傳統 contract:provider 寫 spec(OpenAPI/Swagger),consumer 自己看著辦對接 — 結果是「文件寫的跟實作不一樣」,或「consumer 沒注意 spec 改了」。

Consumer-Driven 翻過來:**消費者寫他真實會打的 request 與期待的 response**,provider 必須通過所有 consumer 的契約才能上線。
- ✅ Provider 不能改任何 consumer 還在依賴的東西(否則 pact 紅)
- ✅ Provider 可以放心加新欄位(老 consumer 不會壞,因為他們沒在 contract 裡 assert 那些)
- ✅ Provider 看得到「真實的 consumer 期待」,而不是 spec 上的理論

Pact Broker 把多個 consumer 的契約集中管理,Provider 跑一次驗證就驗了所有人。

### 6.6 三層契約怎麼分工

| 適用場景 | 哪一層 | 工具 |
|---|---|---|
| 「我換了 InMemory 跟 Real Adapter,測試還會綠嗎?」 | Layer 1 | 抽象契約類 + 兩個子類(JUnit) |
| 「我改了 OrderCreatedEvent 的欄位,別的 Context 還能 deserialize 嗎?」 | Layer 2 | JSON schema snapshot(Jackson) |
| 「我改了 REST response,所有 client 還能解析嗎?」 | Layer 3 | Pact provider verification |

三層用同一個哲學:**把「相容性」寫成測試,而不是寫在 wiki 上**。
...
│  └─ src/test/java/com/tutorial/ecommerce/product/
│     └─ contract/       Port Contract (InMemory + JPA 各跑同一份)
│
├─ payment/              Bounded Context #2:支付(同樣結構 + contract/)
│
├─ inventory/            Bounded Context #3:庫存(同樣結構 + contract/)
│
...
│     ├─ architecture/   ArchUnit 規則(分層、Context 邊界、命名)
│     ├─ modulith/       Spring Modulith 自動驗證 + Documenter
│     ├─ e2e/            6 容器 SharedContainers + E2E + Saga 測試
│     ├─ pact/           Consumer-Driven Contract (Pact provider verification)
│     └─ bdd/            Cucumber 中文 Gherkin
│
│  └─ src/test/resources/contracts/pacts/  外部 consumer 提供的 Pact JSON 檔
│
│ (Event Schema Contract 在 shared-kernel/src/test/.../contract/)
│
...
合約測試不是金字塔的一層,而是 **橫切** — 任何兩個會解耦的東西之間都應該有契約綁住:
...
```
   ...
   Adapter IT             ←─┘    (InMemory Fake 跟 JPA Real 跑同一份)

   Adapter IT             ←─── Layer 2: Event Schema Contract
   E2E IT                       (Integration Event JSON 結構鎖死)

   E2E IT                 ←─── Layer 3: Pact Provider Verification
                                (Consumer-Driven REST API 契約)
```

詳見 [§6 核心概念四:合約測試](#6-核心概念四合約測試-contract-testing)。
